...
namespace QQReborn.App.Services
{
    ...
    public class MockSearchService : ISearchService
    {
        ...
        /// <summary>
        /// The fabricated directory (fake friends/groups above) is demo dressing for the
        /// mock backend only. Against a remote backend the search results are LIVE data and
        /// their ids are routable ("f{uin}" reaches a real SendFriendMessage) -- merging
        /// fake entries there would let the user "message" 健身教练 and fire a real Tencent
        /// API call at whoever owns that made-up uin.
        /// </summary>
        private bool IncludeFakeDirectory => _chat is MockChatService;
        ...
        public async Task<IReadOnlyList<SearchSection>> SearchAsync(string query)
        {
            ...
            foreach (var c in allContacts.Values
                         .Where(c => ContactMatches(c, q))
                         .OrderByDescending(c => Contains(c.Name, q) || Contains(c.Remark, q))
                         .ThenBy(c => c.DisplayName, StringComparer.Ordinal))
            {
                var title = c.DisplayName;
                contactHits.Add(new SearchResult
                {
                    ...
                    {
                        // Must match the "f{uin}" scheme RemoteChatService/RealServer route
                        // sends by -- contacts here can be real (pulled from _chat.GetContactsAsync
                        // above), so this can't be an arbitrary placeholder id.
                        Id = "f" + c.Uin,
            ...
            foreach (var g in allGroups.Values
                         .Where(g => Contains(g.Title, q) || Contains(g.Preview, q) || Contains(g.Id, q))
            ...
            // ---- 聊天记录 (message text) ----
            // Scan ONLY the session-local cache (localOnly: true). Calling the normal
            // GetMessagesAsync against remote would one-shot cloud-backfill every empty
            // conversation, stampede the sign server, and freeze search for minutes.
            var messageHits = new List<SearchResult>();
            const int MaxMessageHits = 50;
            foreach (var conv in conversations)
            {
                if (messageHits.Count >= MaxMessageHits) break;
                IReadOnlyList<ChatMessage> msgs;
                try
                {
                    msgs = await _chat.GetMessagesAsync(conv.Id, localOnly: true);
                }
                catch (Exception)
                {
                    // One conversation's cache failed to load -- skip it rather than aborting.
                    continue;
                }
        ...
        private static bool ContactMatches(Contact c, string q)
        {
            if (c == null) return false;
            if (Contains(c.Name, q) || Contains(c.Remark, q) || Contains(c.Signature, q)) return true;
            // QQ number substring match (e.g. typing "2901")
            return c.Uin.ToString().IndexOf(q, StringComparison.Ordinal) >= 0;
        }

...
{
    /// <summary>Last non-success sign diagnostic (e.g. "HTTP 401 Unauthorized" / "server code …").
    /// Read by RealServer when a send returns sequence 0 so the UI can show a real reason
    /// instead of a silent fake success. Not thread-strict; last-writer wins under concurrent
    /// sign calls, which is good enough for "what just went wrong".</summary>
    public static string? LastFailureReason { get; private set; }

    // Commands that require a signature. Synced verbatim from Lagrange.Core/Common/BotSignProvider.cs
    // (WhiteListCommand) at LagrangeV2 commit 7bfce8a83786275b51fede876fdbc633b23f3525.
    ...
    /// <summary>Serialize sign POSTs. The public signer returns HTTP 401 under concurrent
    /// load (history backfill storms), which made every subsequent PbSendMsg fail and
    /// look like "can't send to anyone". One-at-a-time is slower for bulk OIDB but
    /// keeps send/login signatures reliable.</summary>
    ...
    {
        string? rawBody = null;
        await _signGate.WaitAsync().ConfigureAwait(false);
        try
        {
            var endpoint = $"{_url}/api/sign/sec-sign";
            ...
            Console.WriteLine($"[TokenSignProvider] POST {endpoint} cmd={cmd} uin={(uin == 0 ? _uin : uin)} bodyLen={body.Length} token={(string.IsNullOrEmpty(_token) ? "none" : "present")}");
            // One retry on 401 even while serialized: occasional flap still happens.
            HttpResponseMessage? response = null;
            for (var attempt = 0; attempt < 2; attempt++)
            {
                response?.Dispose();
                using var attemptReq = new HttpRequestMessage(HttpMethod.Post, endpoint);
                if (!string.IsNullOrEmpty(_token))
                    attemptReq.Headers.Authorization = new AuthenticationHeaderValue("Bearer", _token);
                attemptReq.Content = new StringContent(payload, Encoding.UTF8, MediaTypeNames.Application.Json);
                response = await _client.SendAsync(attemptReq).ConfigureAwait(false);
                if (response.IsSuccessStatusCode) break;
                if ((int)response.StatusCode != 401 || attempt == 1) break;
                Console.WriteLine($"[TokenSignProvider] sign [{cmd}] HTTP 401, retrying once…");
            ...
            using (response)
            {
                if (!response!.IsSuccessStatusCode)
                {
                    ...
                    if ((int)response.StatusCode == 401)
                        detail += "（API Key 无效/过期/被限流，请到设置检查签名 token）";
                    Warn(cmd, detail);
                ...
                var result = JsonSerializer.Deserialize<SignerResponse>(rawBody);
                if (result is null)
                {
                    Warn(cmd, $"empty/invalid response. raw={rawBody}");
                    LastFailureReason = "empty/invalid sign response";
                    return null;
                }
                if (result.Code != 0)
                {
                    Warn(cmd, $"server code {result.Code}: {result.Message}. raw={rawBody}");
                    LastFailureReason = $"sign code {result.Code}: {result.Message}";
                    return null;
                }
                if (string.IsNullOrWhiteSpace(result.Value.SecSign))
                {
                    Warn(cmd, $"returned empty sec_sign. message={result.Message}. raw={rawBody}");
                    LastFailureReason = "empty sec_sign";
                    return null;
                }

                if (TryFromHex(result.Value.SecSign, cmd, "sec_sign", rawBody) is not { } secSign) return null;
                var secToken = string.IsNullOrWhiteSpace(result.Value.SecToken)
                    ? Array.Empty<byte>()
                    : TryFromHex(result.Value.SecToken, cmd, "sec_token", rawBody);
                if (secToken is null) return null;
                var secExtra = string.IsNullOrWhiteSpace(result.Value.SecExtra)
                    ? Array.Empty<byte>()
                    : TryFromHex(result.Value.SecExtra, cmd, "sec_extra", rawBody);
                if (secExtra is null) return null;

                LastFailureReason = null;
                Console.WriteLine($"[TokenSignProvider] sign [{cmd}] ok (sign={secSign.Length}B token={secToken.Length}B extra={secExtra.Length}B)");
        ...
        {
            var msg = $"failed: {e.GetType().Name}: {e.Message}";
            if (e.InnerException != null) msg += $" inner={e.InnerException.GetType().Name}: {e.InnerException.Message}";
            msg += $". raw={rawBody ?? "(no response body)"}";
            Warn(cmd, msg);
    ...
    private void Warn(string cmd, string detail)
    {
        var line = $"[TokenSignProvider] sign [{cmd}] {detail}";
        Console.WriteLine(line);
        try { Context?.LogWarning(nameof(TokenSignProvider), $"sign [{cmd}] {detail}"); }
        catch { /* Context may be unset during early dispose; console line above is enough */ }
    }

    // Returns the parsed bytes, or null (with a logged diagnostic) if the value isn't hex.
    private byte[]? TryFromHex(string hex, string cmd, string fieldName, string rawBody)
    {
        try { return Convert.FromHexString(hex); }
        catch
        {
            Warn(cmd, $"{fieldName} not valid hex (len={hex.Length}, prefix='{(hex.Length >= 4 ? hex[..4] : hex)}'). raw={rawBody}");
            return null;
        }
    }
    ...
    private sealed class SignerResponse
    {
        ...
        [JsonPropertyName("value")] public required SecSignValue Value { get; init; }
    }

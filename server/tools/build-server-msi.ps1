...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File server\tools\build-server-msi.ps1
#   powershell -ExecutionPolicy Bypass -File server\tools\build-server-msi.ps1 -SkipPublish
#   powershell -ExecutionPolicy Bypass -File server\tools\build-server-msi.ps1 -Version 0.1.0.3
#   powershell -ExecutionPolicy Bypass -File server\tools\build-server-msi.ps1 -NapCatShell "D:\NapCat.Shell"
...
function Copy-NapCatShell {
    ...
    $tpl = Join-Path $root "server\installer\ServerHost\napcat-config"
...
$publishDir = Join-Path $root "server\publish\ServerHost"
$msiDir = Join-Path $root "server\publish\msi"
$wxs = Join-Path $root "server\installer\ServerHost\Package.wxs"
...
if (-not $SkipPublish) {
    ...
    foreach ($cfgName in @("appsettings.json")) {
...
$tmpWxsDir = Join-Path $root "server\installer\ServerHost\_build"
...
$licenseSrc = Join-Path $root "server\installer\ServerHost\License.rtf"

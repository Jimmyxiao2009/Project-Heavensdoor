...
if ($Publish) {
    ...
    $out = Join-Path $root "server\publish\ServerHost"
    ...
    & (Join-Path $root "server\tools\build-server-msi.ps1") -SkipPublish -SkipMsi -Version 0.1.0.3

    Write-Host ""
    Write-Host "Done. Run: $out\QQReborn.ServerHost.exe"
    Write-Host "MSI: powershell -ExecutionPolicy Bypass -File server\tools\build-server-msi.ps1"
